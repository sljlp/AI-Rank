...
def main(args):
    ...
    logger.info("train dataset len: %d" % (len(train_dataset)))
    os.environ['SAMPLES_PER_EPOCH'] = str(len(train_dataset))
...
if __name__ == '__main__':
    args = parse_args()
    logger.info('AI-Rank-log %s load_data, checksum:%s' % (time.time(), args.checksum))
    logger.info('AI-Rank-log %s test_begin' % (time.time()))
    import numpy as np
    np.save("start_time.npy", [time.time()])
