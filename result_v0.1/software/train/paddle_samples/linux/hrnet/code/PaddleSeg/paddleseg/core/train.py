        ...
        for data in loader:
            ...
            if (iter % save_interval == 0
                ...
                try:
                    ...
                    num_gpus = 1
                samples_pre_epoch = int(os.environ['SAMPLES_PER_EPOCH'])

                logger.info('AI-Rank-log %s eval_accuracy:%s, total_epoch_cnt:%.2f' % (time.time(), mean_iou * 100, float(iter) * batch_size * num_gpus / samples_pre_epoch))
                model.train()
            flag_done = paddle.to_tensor(0, dtype=paddle.int32)
            if (iter % save_interval == 0 or iter == iters) and local_rank == 0:
                ...
                try:
                    ...
                    topVal = 78.5
                if val_dataset is not None and mean_iou * 100 > topVal:
                    flag_done = paddle.to_tensor(1, dtype=paddle.int32)
                    import numpy as np
                    start_time = np.load("start_time.npy")[0]
                    logger.info('AI-Rank-log %s target_quality_time:%.2fsec' % (time.time(), time.time() - start_time))
                    logger.info('AI-Rank-log %s test_finish' % time.time())
                    logger.info('AI-Rank-log %s total_use_time:%.2fsec' % (time.time(), time.time() - start_time))
                    logger.info('AI-Rank-log %s avg_ips:%.2fimages/sec' %(time.time(), iter * batch_size * num_gpus / (time.time() - start_time)))
            if dist.get_world_size() > 1:
                dist.broadcast(flag_done, 0)
            batch_start = time.time()
            if flag_done.numpy() == 1:
                iter = iters
                break
